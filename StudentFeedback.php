...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if ((isset($an1)) and (isset($an2)) and (isset($an3) and (isset($an4)))) {
    $query = "SELECT * FROM Annon_feed WHERE  username = 'uName'";
    $result = mysqli_query($conn, $query);
    if ($result) {
      $rowcount = mysqli_num_rows($result);
      if ($rowcount > 0) {
        $_SESSION['error'] = "You have already filled out feedback!";
      } else {
        $addFeed1 = "INSRT INTO Annon_feed VALUES ('$uName','$an1','$an2','$an3','$an4')";
